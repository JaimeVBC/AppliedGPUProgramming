...
{
	const long i = blockIdx.x * blockDim.x + threadIdx.x; //Column in the grid and matrix
	const long j = blockIdx.y * blockDim.y + threadIdx.y; //Row in the grid and matrix
	float val = 0.0;
...
void naive_sgemm(float *C, float *A, float *B, long size)
{
	struct timeval t0, t1;
	gettimeofday(&t0, NULL);
	naive_sgemm_kernel KERNEL_ARGS2(grid, block)(C, A, B, size);
...
__global__ void shared_sgemm_kernel(float *C, float *A, float *B, long size)
{
	const long col = blockIdx.x * blockDim.x + threadIdx.x;
	const long row = blockIdx.y * blockDim.y + threadIdx.y;
	float val = 0.0;

	/* TODO declare shared memory with size TILE_SIZE x TILE_SIZE */ // DONE
...
void shared_sgemm(float *C, float *A, float *B, long size)
{
	struct timeval t0, t1;
	gettimeofday(&t0, NULL);
	shared_sgemm_kernel KERNEL_ARGS2(grid, block)(C, A, B, size);
	checkCudaErrors(cudaPeekAtLastError());
	checkCudaErrors(cudaDeviceSynchronize());
	gettimeofday(&t1, NULL);
