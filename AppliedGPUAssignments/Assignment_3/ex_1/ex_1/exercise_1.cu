...
// We need this because Visual Studio doesn't recognize the sintax
#ifdef CUDACC
...
__global__ void gpu_grayscale(int width, int height, float* image, float* image_out)
{
    ...
    ///////////////////////////////////////////////////////////
    int idBlock = threadIdx.y * blockDim.x + threadIdx.x;
    int idx = idBlock + blockDim.y * blockDim.x * (blockIdx.x + blockIdx.y * gridDim.x);
    //int row = blockIdx.y * blockDim.y + threadIdx.y;
    //int col = blockIdx.x * blockDim.x + threadIdx.x;

    if (idx < width * height)
    {
        int offset_out = idx; // 1 color per pixel
        //int offset = offset_out * 3; // 3 colors per pixel

        float* pixel = &image[offset_out * 3];
...
__device__ float gpu_applyFilter(float* image, int stride, float* matrix, int filter_dim)
{
    ...
    /* It does make sense because cpu_applyFilter "lives" only in the cpu, and the gpu cannot access to it,
    so it needs another version that can be called from the GPU "__device__". 
    */
...
__global__ void gpu_gaussian(int width, int height, float* image, float* image_out)
{
    ...
    int idThreadInsideBlock_t = threadIdx.y * blockDim.x + threadIdx.x;
    int idThreadInsideBlock_SH = threadIdx.y * BLOCK_SIZE_SH + threadIdx.x;
    ...
    if (idThreadInsideBlock_t == 0) 
    {
        // If the block is not in the last row of blocks... we add the two extra rows  
        if (blockIdx.y < (gridDim.y - 1))
        {
            int offset_aux1 = (row + BLOCK_SIZE) * width + (col);
            int offset_aux2 = (row + BLOCK_SIZE + 1) * width + (col);

            //Fill the extra spaces of the shared block with the original pixels
            for (int i = 0; i < BLOCK_SIZE_SH; i++)
            {
                sh_block[BLOCK_SIZE       * BLOCK_SIZE_SH + i] = image[offset_aux1 + i];
                sh_block[(BLOCK_SIZE + 1) * BLOCK_SIZE_SH + i] = image[offset_aux2 + i];
            }
        }

        // If the block is not in the last column of blocks... we add the two extra columns 
        if (blockIdx.x < (gridDim.x - 1))
        {
            int offset_aux3 = (row)*width + (col + BLOCK_SIZE);
            int offset_aux4 = (row)*width + (col + BLOCK_SIZE + 1);

            //Fill the extra spaces of the shared block with the original pixels
            for (int i = 0; i < BLOCK_SIZE_SH; i++)
            {
                sh_block[BLOCK_SIZE     + i * BLOCK_SIZE_SH] = image[offset_aux3 + i * width];
                sh_block[BLOCK_SIZE + 1 + i * BLOCK_SIZE_SH] = image[offset_aux4 + i * width];
            }
        }
    }
...
void cpu_sobel(int width, int height, float* image, float* image_out)
{
    ...
    for (int h = 0; h < (height - 2); h++)
    {
        ...
        for (int w = 0; w < (width - 2); w++)
        {
            ...
            image_out[offset + (w + 1)] = sqrt(gx * gx + gy * gy);
        }
    }
}
...
__global__ void gpu_sobel(int width, int height, float* image, float* image_out)
{
    ...
    if (threadIdx.x < BLOCK_SIZE && threadIdx.y < BLOCK_SIZE)
    {
        ...
        float gx = gpu_applyFilter(&sh_block[idThreadInsideBlock_SH], BLOCK_SIZE_SH, sobel_x, 3);
        float gy = gpu_applyFilter(&sh_block[idThreadInsideBlock_SH], BLOCK_SIZE_SH, sobel_y, 3);

        image_out[offset_t] = sqrt(gx * gx + gy * gy);
    }
}
